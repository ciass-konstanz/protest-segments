...
def train_models_segments(
    ...
    cols_missing = [f"coco_{category.lower()}_seg_score_count" for category in cols_missing] + [f"coco_{category.lower()}_seg_score_any" for category in cols_missing] + [f"coco_{category.lower()}_seg_prop_max" for category in cols_missing] + [f"coco_{category.lower()}_seg_prop_sum" for category in cols_missing]
    segments_coco_df = segments_coco_df.sort_index(axis=1, level=1)
    segments_coco_df.columns = [f"coco_{label.lower()}_{var}_{agg}" for var, agg, label in segments_coco_df.columns]
    ...
    cols_missing = [f"lvis_{category.lower()}_seg_score_count" for category in cols_missing] + [f"lvis_{category.lower()}_seg_score_any" for category in cols_missing] + [f"lvis_{category.lower()}_seg_prop_max" for category in cols_missing] + [f"lvis_{category.lower()}_seg_prop_sum" for category in cols_missing]
    segments_lvis_df = segments_lvis_df.sort_index(axis=1, level=1)
    segments_lvis_df.columns = [f"lvis_{label.lower()}_{var}_{agg}" for var, agg, label in segments_lvis_df.columns]
    ...
    images_df = pd.merge(images_df, segments_coco_df, how="left", on="id")
    images_df = pd.merge(images_df, segments_lvis_df, how="left", on="id")
    images_df = images_df.replace(to_replace={col: np.nan for col in images_df if col.startswith("coco_") or col.startswith("lvis_")}, value=0)
    ...
    for mod in methods:
        ...
        for voc in vocabularies:
            ...
            for feat in features:
                if feat == "bin":
                    cols = [f"{voc}_{cat.lower()}_seg_score_any" for cat in categories[voc]]
                elif feat == "count":
                    cols = [f"{voc}_{cat.lower()}_seg_score_count" for cat in categories[voc]]
                elif feat == "area_max":
                    cols = [f"{voc}_{cat.lower()}_seg_prop_max" for cat in categories[voc]]
                elif feat == "area_sum":
                    cols = [f"{voc}_{cat.lower()}_seg_prop_sum" for cat in categories[voc]]

                # get training set
                X_train = images_df.loc[(images_df.split == "train"), cols]
                y_train = images_df.loc[(images_df.split == "train"), "protest"]
                ...
                # gridsearch
                logger.info(f"Starting training model for classification method {mod}, vocabulary {voc} and feature {feat}")
                search = GridSearchCV(
                    model,
                    param_grid=params,
                ...
                results_iter["vocab"] = voc
                results_iter["feature"] = feat
